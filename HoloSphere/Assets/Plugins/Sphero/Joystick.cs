...
public class Joystick : MonoBehaviour {
	...
	public Image puck;								// Joystick puck graphic
	public Image background;						// Joystick background graphic
	...
	void ResetJoystick()
	{
		...
		puck.rectTransform.position = defaultRect.position;
		background.rectTransform.position = defaultBackRect.position;
		lastFingerId = -1;
	}
	...
	void SetJoystickSize() {
		...
		background.rectTransform.position = backgroundRect.position;
		...
		puck.rectTransform.position = puckRect.position;
		
		// Store the default rect for the gui, so we can snap back to it
		defaultRect.position = puck.rectTransform.position;	
		defaultRect.x += transform.position.x * startScreenSize.x;
		defaultRect.y += transform.position.y * startScreenSize.y;
		
		defaultBackRect.position = background.rectTransform.position;
	...
	void Update () {
		...
		else
		{
			for(int i =0; i < count; i++)
			{
                ...
                if (puck.GetPixelAdjustedRect().Contains(touch.position) && firstTouch)
				{
				...
				{	
                    ...
                    puck.transform.position = new Vector2(clampedPosition.x, clampedPosition.y);
