...
public class CalibrateButton : MonoBehaviour {
	...
	public Image buttonBackground;					// Button Background Texture
	public Image buttonForeground;					// Button Foreground Texture
	public Image innerRing;						// Overlay image inner ring
	public Image middleRing;						// Overlay image middle ring
	public Image outerRing;						// Overlay image outer ring
	...
	void SetCalibrateButtonSize() {
		...
		middleRing.sourceRect = new Rect(buttonMid.x-midSize*0.5f,buttonRect.y+(ringSize-midSize)*0.5f,midSize,midSize);
		float innerSize = ringSize*0.9f;
		innerRing.sourceRect = new Rect(buttonMid.x-innerSize*0.5f,buttonRect.y+(ringSize-innerSize)*0.5f,innerSize,innerSize);
	...
	void Update () {
		...
		else
		{
			for(int i =0; i < count; i++)
			{
				...
				if ( buttonBackground.contentRect.Contains( touch.position ) && firstTouch )
				{
					isCalibrating = true;
...
//byte bytes[] = outerRing.image.EncodeToPNG();

//       GUI.DrawTexture(guiOuterRingRect, outerRing);
//       GUI.matrix = matrixBackup;
//	GUIUtility.RotateAroundPivot(-currentHeading, guiRingCenter);
//       GUI.DrawTexture(guiInnerRingRect, innerRing);
		//
		}
	}
